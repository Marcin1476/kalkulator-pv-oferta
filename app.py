...
st.sidebar.divider()
st.sidebar.markdown("### Właściciel aplikacji:")
st.sidebar.write("**Marcin Szymański**")
st.sidebar.caption("Wszelkie prawa autorskie zastrzeżone © 2026")
...
# --- WIDOK GŁÓWNY ---
st.title(f"☀️ Raport PV 2026: {st.session_state.city}")
...
with col_map:
    ...
    st_folium(m, height=250, use_container_width=True, key="map_v_final")
    st.write(f"Dni słoneczne (2025): **{sunny_days}**. Wydajność dachu: **{int(final_roof_corr*100)}%**")
...
if st.button("📥 Pobierz Pełną Ofertę PDF"):
    ...
    pdf.cell(200, 10, "RAPORT TECHNICZNO-EKONOMICZNY PV", ln=True, align='C')
    pdf.set_font("Arial", 'I', 12)
    pdf.cell(200, 10, "Projektant: Marcin Szymanski", ln=True, align='C')
    pdf.ln(10)
    
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(200, 10, "DANE TECHNICZNE:", ln=True)
    ...
    pdf.multi_cell(0, 5, "Wszelkie prawa autorskie zastrzezone. Dokument chroniony prawnie. \nWlasciciel aplikacji i autor opracowania: Marcin Szymanski.")
    
    res_pdf = pdf.output(dest='S').encode('latin-1')
    st.download_button("Zapisz Raport PDF", res_pdf, "Oferta_PV_Marcin_Szymanski.pdf")

# --- STOPKA ---
st.markdown("---")
st.markdown("<center>© 2026 Marcin Szymański | Wszelkie prawa autorskie zastrzeżone</center>", unsafe_allow_html=True)
